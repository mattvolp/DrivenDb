...
namespace Fastlite.DrivenDb.Data.Tests.SqLite
{
   public class Examples : IDisposable
   {
      ...
      public Examples()
      {
         ...
         _accessor = DbFactory.CreateAccessor(
            DbAccessorType.SqLite,
            AccessorExtension.All,
            () => new SQLiteConnection(String.Format("Data Source={0};Version=3;New=True", _filename))
            );

         _accessor.Execute(CREATE_DATABASE_SCRIPT);

         //_accessor.Log = Console.Out;
      }

      [Fact]
      public void ReadValueFromDb()
      {
         var value = _accessor.ReadValue<long>("SELECT 7");
      ...
      {
         var Ids = _accessor.ReadValues<long>("SELECT [Id] FROM [Thing1]");
      ...
      {
         var thing1 = _accessor.ReadEntity<Thing1>("SELECT * FROM [Thing1] WHERE Id = @0", 1);
      ...
      {
         var thing1s = _accessor.ReadEntities<Thing1>("SELECT * FROM [Thing1]");
      ...
      {
         var thing1 = _accessor.ReadIdentity<Thing1, int>(1);
      ...
      {
         var entities = _accessor.ReadEntities<Thing1, Thing2>(
      ...
      {
         var thing1s = _accessor.ReadAnonymous(new { Id = 0L, Name = "", Value = 0L },
            "SELECT * FROM [Thing1]"
            );
      ...
      {
         var thing1 = _accessor.ReadIdentity<Thing1, int>(1);
         var thing2s = _accessor.ReadRelated<Thing1, Thing2>(thing1)
      ...
      public void EnumerationParameterInSql()
      {
         ...
         var thing1s = _accessor.ReadEntities<Thing1>("SELECT * FROM [Thing1] WHERE [Id] IN (@0)", new[] { 1L, 2L, 3L });
      ...
      public void DateTimeParameterInSql()
      {
         ...
         //var thing1s = _accessor.ReadEntities<Thing1>("SELECT * FROM [Thing1] WHERE [Created] > @0", DateTime.MinValue);
      ...
      {
         _accessor.Execute("DELETE FROM [Thing2] WHERE [Id] > @0", 50);
      }

      [Fact]
      public void WriteEntitiesToDb()
      {
         var thing2s = _accessor.ReadEntities<Thing2>("SELECT * FROM [Thing2]")
            .ToList();
         ...
         _accessor.WriteEntities(thing2s);
      ...
      {
         var numbers = _accessor.ReadEntities<NumbersTable>(
      ...
      private class NumbersTable : DbEntity<NumbersTable>, INotifyPropertyChanged
      {
         ...
         [DbColumn(IsDbGenerated = false, IsPrimaryKey = true, Name = "Number0")]
         ...
         [DbColumn(IsDbGenerated = false, IsPrimaryKey = true, Name = "Number1")]
         public int Number1
         {
         ...
         [DbColumn(IsDbGenerated = false, IsPrimaryKey = true, Name = "Number2")]
         public long Number2
         {
      ...
      private class Thing2 : DbEntity<Thing2>, INotifyPropertyChanged
      {
         ...
         [DbColumn(IsDbGenerated = true, IsPrimaryKey = true, Name = "Thing1Id")]
